...
class Dataset(ABC):
    def __init__(self, scale: bool, z_blind: bool) -> None:
        super().__init__()
        self.scale = scale
        self.z_blind = z_blind
    ...
    def split_data(self, fold):
        ...
        for i in range(n):
            ...
            x_chunks.append(self.X.copy().iloc[start:end] if self.z_blind else self.XZ.copy().iloc[start:end])
    ...
    def numpy(self, fold=0):
        train_data, val_data, test_data = self.split_data(fold)
...
class GermanDataset(Dataset):
    def __init__(self, seed: int | None = None, z_blind: bool = True):
        super().__init__(True, z_blind)
    ...
    def set_improvable_features(self):
        self.imp_feats = {
            'U_index': np.setdiff1d(np.arange(self.X.shape[1] if self.z_blind else self.XZ.shape[1]),[0,5,14,16]),
            'C_index': [0,3,7,9],
            'C_min': [0,0,0,0],
            'C_max': [3,4,2,3]
        }
...
class IncomeDataset(Dataset):
    def __init__(self, num_samples: int | None = None, seed: int | None = None, z_blind: bool = True) -> None:
        super().__init__(True, z_blind)
    ...
    def set_improvable_features(self):
        self.imp_feats = {
            'U_index': np.setdiff1d(np.arange(self.X.shape[1] if self.z_blind else self.XZ.shape[1]),[1]),
            'C_index': [1],
            'C_min': [1],
            'C_max': [24]
        }